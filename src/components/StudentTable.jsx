...
import classData from './students.json';

const StudentTable = () => {
    const [classOneStudents, setClassOneStudents] = useState([]);
    const [classTwoStudents, setClassTwoStudents] = useState([]);
    const [classThreeStudents, setClassThreeStudents] = useState([]);

    useEffect(() => {
        // Filter out only "class_1" data
        const classOneData = classData.class_1;
        setClassOneStudents(classOneData);

        // Filter out only "class_2" data
        const classTwoData = classData.class_2;
        setClassTwoStudents(classTwoData);

        // Filter out only "class_3" data
        const classThreeData = classData.class_3;
        setClassThreeStudents(classThreeData);
    }, []);
